...
import { isCorrected, isStringLiteral, } from "../generated/ast.js";
import { createVibeServices } from "../vibe-module.js";
import { VibePlanSchema } from "../resolver/schemas.js";
...
function extractFields(corrected) {
    const out = {};
...
export async function runPipeline(input) {
    ...
    for (const region of stream.regions) {
        if (region.kind === "structured") {
            ...
            if (!input.proseSchema)
                continue; // pipeline configured to ignore prose
    ...
    for (const r of resolvedRegions) {
        try {
            ...
            if (val && (val.kind === "plan" || val.session)) {
                const plan = VibePlanSchema.parse(val);
