...
const SYSTEM_PROMPT = `You are the Pawfall action-plan-review lane.
...
3. Call invoke_codex_cli with a prompt that includes the action plan content inline and asks for the top 2-3 priority items to start with, with one-sentence rationale each. Keep prompt under 4000 chars.
4. Call invoke_claude_cli with a prompt that includes the action plan content inline and asks for the 3 biggest risks or contradictions, with one-sentence rationale each. Keep prompt under 4000 chars.
...
export function buildCliDelegationAgent() {
  const model = createCerebrasModel();

  return createDeepAgent({
    model,
    systemPrompt: SYSTEM_PROMPT,
    tools: [invokeCodexCli, invokeClaudeCli],
    ...
    permissions: [
      {
        operations: ["read"],
        paths: ["/outputs/2026-05-15-action-plan.md"],
        mode: "allow",
      },
      {
        operations: ["write"],
        paths: ["/outputs/action-plan-review.md"],
        mode: "allow",
      },
      {
