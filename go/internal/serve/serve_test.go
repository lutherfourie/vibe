...
func TestProvidersReturnsDefaultOpenAICompatibleProviders(t *testing.T) {
	...
	wantProviders := []string{"cerebras", "fake", "grok", "openai"} // "grok" registered for M2 multi-provider dispatch; "claude" temporarily disabled to avoid interfering with another local project using claude CLI
	if !reflect.DeepEqual(got.Providers, wantProviders) {
		t.Fatalf("providers = %#v, want %#v", got.Providers, wantProviders)
	}
	if got.Default != "fake" {
